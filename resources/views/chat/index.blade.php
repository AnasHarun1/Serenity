<x-chat-layout>
    ...
    <div
        ...
        <div
            ...
            <!-- Header -->
            <div class="px-6 py-4 border-b border-earth-100 flex justify-between items-center bg-beige-50/50 backdrop-blur-sm z-20 shrink-0">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form action="{{ route('chat.clear') }}" method="POST" onsubmit="return confirm('Hapus semua percakapan?');">
                        @csrf
                        <button class="group flex items-center gap-2 px-3 py-2 md:px-4 md:py-2 rounded-full bg-beige-100 text-earth-600 hover:bg-terracotta-50 hover:text-terracotta-600 transition border border-transparent hover:border-terracotta-100">
                            <span class="text-xs font-bold uppercase tracking-wider hidden sm:block">Reset</span>
                            <svg class="w-4 h-4 group-hover:rotate-90 transition duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div x-data="{
                    ...
                    @if(session('chat_mode', 'ai') === 'expert')
                        <div class="sticky top-0 z-20 mb-6 flex justify-center">
                            <div class="bg-blue-50 border border-blue-200 text-blue-800 text-xs px-4 py-2 rounded-full shadow-sm flex items-center gap-2">
                                ...
                                Terhubung dengan Konselor Asli. Membalas pada jam operasional 09.00 - 17.00.
                            </div>
                        </div>
                    @endif
                ...
                <div class="pt-4 px-4 pb-[max(1.5rem,env(safe-area-inset-bottom))] md:p-6 bg-white/80 border-t border-earth-100 relative z-20 backdrop-blur-md shrink-0">
                    ...
                    <form @submit.prevent="sendMessage" class="relative group max-w-4xl mx-auto flex items-end gap-3">
                        ...
                        <div class="flex-1 relative bg-beige-50 rounded-3xl border border-earth-100 focus-within:border-terracotta-300 focus-within:bg-white transition-all duration-300 shadow-inner flex items-center">
                            <input id="messageInput" type="text" x-model="newMessage" :disabled="isLoading || {{ $disableInput ? 'true' : 'false' }}"
                                autocomplete="off" required 
                                placeholder="{{ $disableInput ? 'Menunggu respons ahli... Layanan aktif pkl 09.00.' : 'Ceritakan harimu...' }}"
                        ...
                        <button type="submit" :disabled="isLoading || {{ $disableInput ? 'true' : 'false' }}"
